...
if($action == 'test') {
	...
	//同步登录 API 接口
	$db2 = new dbstuff;
	$db2->connect($host, $dbuser, $dbpsw, $dbname, $pconnect);
	unset($host, $dbuser, $dbpsw, $dbname, $pconnect);

	$uid = intval($get['uid']);
	$query = $db2->query("SELECT uid, username FROM {$tablepre}members WHERE uid='$uid'");
	if($member = $db2->fetch_array($query)) {
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		dsetcookie('qtwy', authcode($member['uid']."\t".$member['username'], 'ENCODE'), 86400 * 365);
	}
